...
<head>
  ...
  <title class="counttitle">SponsoRise Admin</title>
               ...
               foreach($fetch_data->result() as $row)  
                ...
                <tr id="<?php echo $row->idSpons; ?>" class="tri" onmouseover="overtr(this)" onmouseout="notovertr(this)">  
                    ...
                    ?><img src="<?php echo base_url().'blue-tick.png' ; ?>" style="width:20px;height:20px;"/><?php } ?></td>  
                     ...
                     <td data-toggle="modal" data-target="#myModal" style="cursor:grab;width:20px">
                     <?php  echo '<img src="data:image/jpeg;base64,'.$row->img.'" class="img-thumbnail"/>'; ?>
                      </td>
                      <td><input type="button" value="activé" id="<?php  echo $row->idSpons; ?>" class="activerbutt btn" style='background-color:#9c27b0;color:#fff'/></td>
                     <td><button href="#" id="<?php echo $row->idSpons; ?>" class="deletebutt btn" style='background-color:#9c27b0;color:#fff'>Supprimer</button></td>
                </tr>  
                  <?php  }  
                  else {
                    ?>
                <tr id="<?php echo $row->idSpons; ?>" class="tri" onmouseover="overtr(this)" onmouseout="notovertr(this)">  
                    ...
                    <?php }?>
                      <img src="<?php echo base_url().'blue-tick.png' ; ?>" id="<?php echo $row->idSpons; ?>" class="tick" style="width:20px;height:20px;display:none"/>
                    </td>  
                     <td><?php echo $row->nomSpons; ?></td>  
                     <td><?php echo $row->prenomSpons; ?></td>  
                     <td><?php echo $row->organismeSpons; ?></td> 
                    ...
                    <?php  echo '<img src="data:image/jpeg;base64,'.$row->img.'" class="img-thumbnail"/>'; ?>
                     ...
                     <td><input type="button" value="désactivé" id="<?php echo $row->idSpons; ?>" class="activerbutt btn" style='color:#fff;background-color:#9c27b0'/></td>
                     <td><button href="#" id="<?php echo $row->idSpons; ?>" style='background-color:#9c27b0;color:#fff' class="deletebutt btn">Supprimer</button></td>
                </tr>  
                  <?php
                  }   
                }
...
function notovertr(x) {
  ...
  $(document).ready(function(){  
    $('.tri').click(function(e){  
        ...
        var id = $(this).attr("id");
        $.ajax({  
            url:"<?php echo base_url(); ?>index.php/welcome/upload_modal",
            method:"POST",  
            data:{id:id},
            success:function(data)  
            {
                ...
                $('#validermodalbutt').click(function(e){
                    $.ajax({  
                        url:"<?php echo base_url(); ?>index.php/welcome/valider_data",
                        method:"POST",  
                        data:{id:id},
                        success:function(data)  
                        {
 ...
 function load_unseen_notification(view = '')
 {
   ...
   success:function(data)
   {
    ...
    if(data.unseen_notification > 0)
    {
     ...
     $('.counttitle').html("SponsoRise Admin ("+data.unseen_notification+")");
    }
   }
  });
 }

 function load_unseen_notificationPop(view = '')
 {
   $.ajax({
   url:"<?php echo base_url(); ?>index.php/welcome/notif_adminPop",
   method:"POST",
   data:{view:view},
   dataType:"json",
   success:function(data)
   {
    $('.content1').html(data.notification);
   }
  });
 }
