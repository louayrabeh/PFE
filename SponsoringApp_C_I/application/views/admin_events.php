        ...
        <div class="container-fluid">
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold " style='color:#9c27b0'>Listes des évènements</h6>
            ...
            <div class="card-body">
              <div class="table-responsive">
                ...
                <thead>
                  ...
                  <tbody>
                  <?php
               foreach($fetch_dataEvents->result() as $row)  
                {
                  //if ($row->isactive == "1") { ?>
                <tr id="<?php echo $row->idEvent; ?>" class="tri" onmouseover="overtr(this)" onmouseout="notovertr(this)">  
                
                     <td><?php echo $row->idEvent; ?> <?php
                    ...
                    foreach($q1->result() as $row1)  
                {
                  if($row1->isvalid)
                    ...
                    ?><img src="<?php echo base_url().'blue-tick.png' ; ?>" style="width:20px;height:20px;"/><?php } } ?></td>  
                     ...
                     <?php echo '<div  style="margin:2%;text-align:center"><img src="data:image/png;base64,'.$row->URL_Image.'" class="img-thumbnail" style="width:40px;height:40px"/></div>'; ?>
                      </td> 
                     <?php
                     ...
                     { 
                    ?>
                     <td><?php echo $row1->nomOrg." ".$row1->prenomOrg; ?></td>
                     
                <?php
                }?>
                     <td><button href="#" id="<?php echo $row->idEvent; ?>" class="deletebutt btn" style='background-color:#9c27b0;color:#fff'>Supprimer</button></td>
                </tr>  
                 <?php 
                }
                ?>
...
<div class="modal" id="myModal">
  <div class="modal-dialog">
    <div class="modal-content" style="height:600px;color:black;overflow: auto;background-color:#fff"><div style="text-align:right;margin-top:2%;margin-right:2%"><i data-dismiss="modal" class="fa fa-times"></i></div>
...
<script>
  $(document).ready(function(){
      $(".deletebutt").click(function(){
        ...
        {
          $.ajax({
           url:'<?php echo base_url(); ?>index.php/welcome/deleteEvent',
           method:'POST',
           data:{id:id},
           success:function(){
...
function notovertr(x) {
  ...
  $(document).ready(function(){  
    $('.tri').click(function(e){  
        ...
        console.log(id);
        $.ajax({  
            url:"<?php echo base_url(); ?>index.php/welcome/upload_modalEvent",
            method:"POST",  
            data:{id:id},
            success:function(data)  
            {
                ...
                $('#validermodalbutt').click(function(e){
                  action=$('#validermodalbutt').val();
                    $.ajax({  
                        url:"<?php echo base_url(); ?>index.php/welcome/validerEvent",
                        method:"POST",  
                        data:{id:id,action:action},
                        success:function(data)  
                        {
                          console.log(action);
                            $('tr#'+id+'').css('background-color', '#9c27b0');
                            $('tr#'+id+'').fadeOut('slow');
                        }  
                    }); 
                });
